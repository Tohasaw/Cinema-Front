...
<div class="admin-content">
  ...
  <div class="admin-container">
    ...
    <div class="subcont">
      ...
      <div class="title">
        <p>Просмотр таблицы «Прайс-листы»</p>
        <div class="buttons">
      ...
      <table class="data">
        ...
        <tbody>
          <tr v-for="(row, index) in rows" :key="index">
            ...
            <td>
              <router-link :to="{path: 'pricelists/'+ row.id}">
                  <svg-icon type="mdi" :path="magnify" color="#5E7CE7"></svg-icon>
              </router-link>
            </td>
            <td>
              <router-link :to="{path: 'seatprices/'+ row.id}">
                  <svg-icon type="mdi" :path="magnify" color="#5E7CE7"></svg-icon>
              </router-link>
            </td>
            <td>
...
export default {
  ...
  computed: {
    rows() {
      if (this.$store.state.entities.pricelists !== null) {
        let entries = this.$store.state.entities.pricelists;
  ...
  watch: {
    rows() {
      this.pages = Math.floor(this.$store.state.entities.pricelists.length / this.maxItems);
      if (this.$store.state.entities.pricelists.length % this.maxItems !== 0) {
        this.pages += 1;
      }
    },
  },
