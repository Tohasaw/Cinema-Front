...
export default {
  ...
  computed: {
    rows() {
      if (this.$store.state.entities.pricesforlist !== null) {
        let entries = this.$store.state.entities.pricesforlist;
  ...
  async created() {
    ...
    await this.$store.dispatch('entities/getData', 'Prices');
  },
  watch: {
    rows() {
      this.pages = Math.floor(this.$store.state.entities.pricesforlist.length / this.maxItems);
      if (this.$store.state.entities.pricesforlist.length % this.maxItems !== 0) {
        this.pages += 1;
      }
    },
  },
  methods: {
    ...
    updateData(row) {
      ...
      delete finrow.relation;
      this.$refs.createrow.errors = [];
      this.$refs.createrow.entityId = this.id;
      this.$refs.createrow.currentRow.priceListId = this.id;
      this.$refs.createrow.currentRow = row.relation[0];
      this.$refs.createrow.selectedRow = finrow;
      this.$refs.createrow.name = this.entityName;
    },
    deleteData(row) {
      console.log(row);
