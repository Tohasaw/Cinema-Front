    ...
    <div class="modal">
      ...
      <div class="back">
        <div class="modal-content">
          <div class="prices">
            <div class="unabled"></div>
            <div class="text">Доступно</div>
            <div class="disabled"></div>
            <div class="text">Недоступно</div>
          </div>
          <div class="selecter">
            Выберите одно или более мест
            <button class="submit-button" @click="submitSeats()">Сохранить</button>
          </div>
...
export default {
  ...
  methods: {
    getColor(row, column) {
      let seat = this.seats.find(s => s.row === row && s.column === column);
      if (this.selectedSeats.find(s => s === seat.id)) {
        if (seat.isAvailable === true) {
          return { 'background-color': 'white', 'border-radius': '15px' };
        } else {
          return { 'background-color': 'rgb(122, 86, 86)', 'border-radius': '15px' };
        }
      } else {
        if (seat.isAvailable === true) {
          return { 'background-color': 'white' };
        } else {
          return { 'background-color': 'rgb(122, 86, 86)' };
        }
      }
    },
    addToList(row, column) {
      let seat = this.seats.find(s => s.row === row && s.column === column);
      if (this.selectedSeats.find(s => s === seat.id) !== undefined) {
        let selectedIndex = this.selectedSeats.indexOf(seat.id);
    ...
    submitSeats() {
      if (this.selectedSeats.length > 0) {
        this.$store.dispatch('entities/putRangeData', { data: JSON.parse(JSON.stringify(this.selectedSeats)), name: 'Seats' });
        this.selectedSeats = [];
      }
    }
  },
