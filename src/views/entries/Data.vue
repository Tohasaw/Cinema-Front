...
<div class="admin-content">
  ...
  <div class="admin-container">
    ...
    <div class="subcont">
      ...
      <div class="title">
        <p>Просмотр таблицы «Расписание»</p>
        <div class="buttons">
      ...
      <table class="data">
        ...
        <tbody>
          <tr v-for="(row, index) in rows" :key="index">
            ...
            <td>{{ row.priceList.name }}</td>
            <td>{{ row.visit }} ({{ row.visitPercent }}%)</td>
            <td>
...
export default {
  ...
  computed: {
    rows() {
      if (this.$store.state.entities.entries !== null) {
        let entries = this.$store.state.entities.entries;
  ...
  async created() {
    ...
    await this.$store.dispatch('entities/getData', 'PriceLists');
    await this.$store.dispatch('entities/getData', 'Movies');
  },
  watch: {
    rows() {
      this.pages = Math.floor(this.$store.state.entities.entries.length / this.maxItems);
      if (this.$store.state.entities.entries.length % this.maxItems !== 0) {
        this.pages += 1;
      }
    },
  },
  methods: {
    ...
    createData() {
      ...
      this.$refs.createrow.selectedMovieComp = '';
      this.$refs.createrow.selectedPriceComp = '';
      window.scrollTo({
        top: 103,
        behavior: 'smooth'
      });
